...
" Key Mappings / Commands / Autocmds {
    ...
    " 重读/在新的分屏中打开我的 ~/.vimrc文件:source $MYVIMRC<CR> :vsplit $MYVIMRC<CR>
    ...
    augroup VimRcAUs
      ...
      autocmd FileType qf setl nobl nolist nowrap nospell nu nornu
            ...
            \| nnoremap <buffer> <leader>m <Cmd>if &modifiable<Bar>setl noma nomod<Bar>else<Bar>setl ma<Bar>endif<CR>
            \| nnoremap <buffer> <leader>u <Cmd>cgetbuffer<CR>:cclose<CR>:copen<CR>
            \| nnoremap <buffer> <leader>r <Cmd>cdo s/// \| update<C-Left><C-Left><Left><Left><Left>
